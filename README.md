...
**Token Counting & Cost Estimation (12 MCP tools)**

## Overview

PyTokenCalc is part of the unified **MCP 2.0 Mega-Platform** (207 tools across 18 projects). This project provides AI-native tools via Model Context Protocol (MCP 2.0).

## Features

- **MCP 2.0 Support**: Discoverable via MCP protocol protocol on port 8778
...
```bash
pip install --only-binary=:all: PyTokenCalc
```

## MCP 2.0 Integration

Enable MCP tools on port **8778** (see MCP_QUICKSTART.md for details).
...
See [MCP_QUICKSTART.md](PyTokenCalc/MCP_QUICKSTART.md) for detailed tool documentation.
