...
- **A network failure while loading tiktoken's OpenAI encodings can take down the entire token-counter registry**, not just OpenAI counting — `OpenAITokenCounter.__init__` (`pytokencalc/tokenizers/openai_counter.py:66`) raises `RuntimeError` on a failed download, and `TokenCounterRegistry` (`pytokencalc/tokenizers/registry.py:39`) only catches `ImportError`, so the `RuntimeError` propagates and breaks Anthropic/Google/Cohere/HuggingFace/Ollama/custom-provider counting too. Reproduced live while validating this repo (see [ROADMAP_HONEST.md](ROADMAP_HONEST.md#2-critical-bug-found-in-this-pass-not-fixed--needs-a-dedicated-session) for the full trace). Not yet fixed.
...
- Lint (`ruff`/`black`/`mypy`) is configured in `pyproject.toml` and `.pre-commit-config.yaml` but is not run in CI; `ruff check pytokencalc/` currently reports 291 findings against ruff's default rule set.
